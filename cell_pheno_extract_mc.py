...
panel_dir = download_dir+"/MC_IL4_mDC_Th2_Panel"
...
cell_pheno_chart=pd.read_excel(panel_dir+"/MC_IL4_mDC_Th2_panel_cell_type_annot_v2.xlsx", index_col=0, engine='openpyxl')
annot_df=pd.read_excel(panel_dir+"/b1_MC_IL4_mDC_Th2_annotation.xlsx")
...
#        tmp_class=[",".join(list(i))+" positive" for i in tmp_markers] # NOTE: Check the QuPath output format
        tmp_class=[": ".join(list(i)) for i in tmp_markers] # NOTE: Check the QuPath output format
#        print(tmp_class)
...
#    print(tmp_df['Class'].value_counts())
    ...
    tmp_df['IL4']=np.where(tmp_df['Name'].str.contains('IL4'), "Positive", "Negative")
    tmp_df.loc[tmp_df['Class'].isna(),'phenotype']='Unknown'
    img_mask = annot_df['Image'] == tmp_df['Image'].unique()[0]
#    print(tmp_df.loc[tmp_df['IL4'] == "Positive"])
    tmp_df['Area um2']=annot_df['Area Âµm^2'].loc[img_mask.index[img_mask]].tolist()[0]
    tmp_df['Perimeter um']=annot_df['Perimeter Âµm'].loc[img_mask.index[img_mask]].tolist()[0]
...
#    print(tmp_df['phenotype'].value_counts())
    ...
    cln_spt_df=cln_df[['Image', 'Class', 'Centroid X µm', 'Centroid Y µm', 'pid', 'phenotype', 'IL4']]
